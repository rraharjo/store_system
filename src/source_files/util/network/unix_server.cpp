    ...
    while (true)
    {
        ...
        try
        {
            ...
            while (to_send.get_current_payload_len() > 0)
            {
                ...
                if (i_result == -1)
                {
                    throw std::runtime_error("Error on send() with error code " + std::to_string(errno) + "\n");
                }
            }
        }
...
void util::network::TCPServer::init_socket()
{
    ...
    this->listen_socket = socket(this->network_fam, this->socket_type, this->protocol);

    if (this->listen_socket == -1)
    {
        throw std::runtime_error("Error on socket() with error code " + std::to_string(errno) + "\n");
    }

    struct sockaddr_in addr;
    addr.sin_family = this->network_fam;
    addr.sin_port = htons(std::stoi(this->port_number));
    addr.sin_addr.s_addr = inet_addr(this->ip_address.c_str());

    i_result = bind(this->listen_socket, (struct sockaddr *) &addr, sizeof(addr));
    if (i_result == -1)
    {
        throw std::runtime_error("Error on bind() with error code " + std::to_string(errno) + "\n");
    }
}
...
void util::network::TCPServer::start_server_with_driver()
{
    ...
    if (i_result == -1)
    {
        throw std::runtime_error("Error on listen() with error code " + std::to_string(errno) + "\n");
    }
