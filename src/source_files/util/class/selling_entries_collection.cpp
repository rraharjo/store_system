    ...
    namespace baseclass
    {
        ...
        std::vector<std::unique_ptr<HasTable>> SellingEntriesCollection::get_from_database(std::vector<util::TableCondition> &conditions)
        {
            ...
            for (std::vector<std::string> &record : records)
            {
                ...
                std::unique_ptr<inventory::SellingEntry> selling_entry_from_db =
                    std::make_unique<inventory::SellingEntry>(record[0],
                                                              record[item_index],
                                                              record[3],
                                                              std::stod(record[4]),
                                                              std::stoi(record[5]));
                std::unique_ptr<HasTable> to_add((HasTable *)selling_entry_from_db.release());
