...
{
    if (pipe(this->input_pipe) == -1)
    {
        throw std::runtime_error("error on input _pipe()");
    }
    if (pipe(this->output_pipe) == -1)
    {
...
std::string storedriver::PipeIODriver::read_input()
{
    ...
    int bytes_read = 0;
    if (bytes_read = read(this->input_pipe[0], read_buff, STREAM_SIZE) == -1)
    {
...
void storedriver::PipeIODriver::write_input(std::string input)
{
    ...
    if (write(this->input_pipe[1], write_buff, input_length + 1) == -1)
    {
...
std::string storedriver::PipeIODriver::read_output()
{
    ...
    int bytes_read = 0;
    std::string to_ret;
    if (bytes_read = read(this->output_pipe[0], read_buff, STREAM_SIZE) == -1)
    {
...
void storedriver::PipeIODriver::write_output(std::string input)
{
    ...
    char write_buff[STREAM_SIZE];
    strcpy(write_buff, input.c_str());
    if (write(this->output_pipe[1], write_buff, input_length + 1) == -1)
    {
